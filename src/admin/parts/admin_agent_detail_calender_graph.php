...
<div style="display:flex;width:100%;justify-content:center;">
    <div>
        ...
        if ($year_month_parameter_set) {
            ...
            echo 'admin_agent_detail.php?year=' . $decrease_year . '&month=' . $decrease_month;
        }else{
            if (date('n') != 1) {
                //パラメータのmonthが1じゃないなら普通に月減らす
    ...
    <div>
        ...
        <?php
                if ($year_month_parameter_set) {
                    ...
                    echo 'admin_agent_detail.php?year=' . $increase_year . '&month=' . $increase_month;
                }
...
<script>
    for(let index=1;index<=35;index++){
        document.getElementById(`each_day${index}`).addEventListener('click',function(){
            ...
            window.location=`admin_agent_selected_date.php?year=${selected_year}&month=${selected_month}&date=${selected_date}`;
        });
    }
</script>
    <?php require "../../agent/parts/agent_graph.php";?>
</section>