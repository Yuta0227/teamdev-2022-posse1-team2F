...
for ($agent_id = ($page_number - 1) * $agents_per_page; $agent_id < $agents_per_page * $page_number; $agent_id++) {
    if ($agent_id  < count($agent_list_array)) {
        ...
        foreach (${"mail_data" . $agent_list_array[$agent_id]['agent_id']} as $assignee) {
            ...
            echo '<div>';
            echo '<div>担当者支店</div>';
            echo '<input value="' . $assignee['agent_branch'] . '"> ';
            echo '</div>';
            echo '<div>';
            echo '<div>担当者氏名</div>';
            echo '<input value="' .$assignee['assignee_name'] . '"> ';
            echo '</div>';
            echo '<div>';
            echo '    <div>特集記事をお願いするメール本文</div>';
            echo '    <textarea cols="30" rows="5">'.${"mail_data" . $agent_list_array[$agent_id]['agent_id']}[0]['agent_name'].${"mail_data" . $agent_list_array[$agent_id]['agent_id']}[0]['agent_branch'] . 'さんへの特集記事招待メールテンプレ</textarea>';
            echo '</div>';
            $index++;
        }
